...
class FormulaStudentv1(gym.Env):
    ...
    def create_obs(self) -> np.ndarray[float]:
        ...
        obs = sensor + car_position + centerline + [velocity, angle]
        assert len(obs) == 33, f"Observation length is {len(obs)}, should be 33"
        return np.array(obs)

    def reset(self, **kwargs):
        maps = ['map11.png']  # ["map.png", "map3.png", "map8.png", "map11.png"]
    ...
    def step(self, action):
        ...
        new_closest_point = self.track.get_center_line_index(
        ...
        score = 20 * delta_index - 2 * delta_steering ** 2 - 5
        done = not in_track
        if done:
            score -= 500
        self.clock.tick(self.clock_speed)
        return obs, score, done, self.step_count > 1000, {}
...
if __name__ == '__main__':
    ...
    file_name = "formula_student_ppo_0993"

    retest = True
    while retest and os.path.exists(f"{file_name}.zip"):
    ...
    if train:
        env = FormulaStudentv1(render_mode="ai", clock_speed=100)
